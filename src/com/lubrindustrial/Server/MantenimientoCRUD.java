...
//                                                        " WHERE a.ACTIVO <> 0 AND a.CANTIDAD_ART <= a.PUNTOREORDEN_ART");
            resultado = conexion.getStmt().executeQuery("SELECT * FROM MANTENIMIENTO WHERE ACTIVO <> 0 AND (FECHAPROGRAMADATERMINO_MANT - NOW()) >= 30");
...
//                                                        " ON (a.ID_PROV = p.ID_PROV)" +
//                                                        " WHERE a.ACTIVO <> 0 AND a.CANTIDAD_ART <= a.PUNTOREORDEN_ART");
            resultado = conexion.getStmt().executeQuery("SELECT * FROM MANTENIMIENTO WHERE ACTIVO <> 0 AND (FECHAPROGRAMADATERMINO_MANT - NOW()) >= 1");
